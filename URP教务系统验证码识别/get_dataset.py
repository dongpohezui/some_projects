...
#爬取图片,dataset_dir验证码素材目录
def get_jpg(number,dataset_dir = 'dataset'):
	print("开始爬取图片\n");
	s=requests.session()
	...
	for i in range(0,number):
		...
		with open(dataset_dir+"/"+code+'.jpg', 'wb') as f:
...
def process_dataset(dataset_dir = 'dataset'):
	...
	joblib.dump((X,y) , 'urp_captcha_train_data.pkl')
	
	print("处理图片结束\n");

if __name__ == "__main__":
	#get_jpg(5000)
	process_dataset("urp_dataset")
